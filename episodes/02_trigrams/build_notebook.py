...
def main():
    ...
    if args.execute:
        ...
        with tempfile.TemporaryDirectory(prefix="episode02-kernel-") as temporary:
            runtime = Path(temporary)
            kernel = runtime / "kernels" / "python3"
            kernel.mkdir(parents=True)
            (kernel / "kernel.json").write_text(json.dumps({
                "argv": [sys.executable, "-m", "ipykernel_launcher", "-f", "{connection_file}"],
                "display_name": "Episode 02 verification", "language": "python",
            }))
        ...
        (HERE / "episode_02.html").write_text(html)
    nbformat.write(notebook, HERE / "episode_02.ipynb")
